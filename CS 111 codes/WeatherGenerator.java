...
public class WeatherGenerator {
    ...
    for (int i = 0; i < drywet.length; i++) {
        double[] row = drywet[i];
        if (longitude == row[0] && latitude == row[1]) {
            for (int m = 0; m < 12; m++) {
                drywetProbability[m] = row[m + 2];
            }
        }
    }
    ...
    public static int bestWeekToTravel(int[] forecast) {    
            ...
            if(forecast.length == 28){
                length = 22; 
            }
            else if(forecast.length == 30){
                length = 24;
            }
    
            else if(forecast.length == 31){
                length = 25;
            }    
            int[] sevenDayWeek = new int[length];

            for(int i = 0; i<sevenDayWeek.length; i++){
                int numOfDryDays = 0;
             for(int j = i; j<=i+6; j++){
                    if(forecast[j] == DRY)
                        numOfDryDays++;
                }
                sevenDayWeek[i] = numOfDryDays;
            }
            int max = sevenDayWeek[0];

            for(int i = 0; i<sevenDayWeek.length; i++){
                if(sevenDayWeek[i]>max){
                    max = sevenDayWeek[i];
                }
            }
            boolean check = false;
            for(int i = 0; i<sevenDayWeek.length;i++){
                if(sevenDayWeek[i] == max && check == false){
                    index = i;
                    check = true;
                }
            }
    ...
    public static void main (String[] args) {
        StdRandom.setSeed(1636171483835L);
